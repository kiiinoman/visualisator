...
def ask_mapping(columns: List[str]) -> Dict[str, Optional[str]]:
...
def main():
    ...
    print()
    print("=" * 60)
    print(f"  ФАЙЛ СОХРАНЁН ({size_kb} KB)")
    print("=" * 60)
    print(f"  {abs_path}")
    print()
    print("  Откройте этот файл в браузере:")
    print(f"  -> Дважды кликните на файл, или")
    print(f"  -> Перетащите файл в окно браузера")
    print("=" * 60)

    if not args.no_open:
        try:
            webbrowser.open(f"file://{abs_path}")
            print("  (попытка открыть браузер автоматически...)")
        except Exception:
            pass
